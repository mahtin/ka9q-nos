...
/* In udp.c: */
extern int del_udp(struct udp_cb *conn);
extern struct udp_cb *open_udp(struct socket *lsocket,
	void (*r_upcall)(struct iface *iface,struct udp_cb *,int));
extern int recv_udp(struct udp_cb *up,struct socket *fsocket,struct mbuf **bp);
extern int send_udp(struct socket *lsocket,struct socket *fsocket,char tos,
	char ttl,struct mbuf **bpp,uint16 length,uint16 id,char df);
extern void udp_input(struct iface *iface,struct ip *ip,struct mbuf **bp,
	int rxbroadcast,int32 said);
extern void udp_garbage(int drastic);

#ifdef HOPCHECK
extern void udp_icmp(int32 icsource, int32 ipsource,int32 ipdest,
	char ictype,char iccode,struct mbuf **bpp);
/* In hop.c: */
extern void hop_icmp(struct udp_cb *ucb, int32 icsource, int32 ipdest,
	uint16 udpdest, char ictype, char iccode);
#endif

/* In udpcmd.c: */
extern int st_udp(struct udp_cb *udp,int n);

/* In udphdr.c: */
extern void htonudp(struct udp *udp,struct mbuf **data,struct pseudo_header *ph);
extern int ntohudp(struct udp *udp,struct mbuf **bpp);
extern uint16 udpcksum(struct mbuf *bp);
